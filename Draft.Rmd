...
linreg1 = res1 |> collect_metrics() |> as.data.frame()
linreg1 = cbind(linreg1[,1], round(linreg1[,3],2))
knn1 = res2 %>% collect_metrics() |> as.data.frame()
knn1 = cbind(knn1[,1], round(knn1[,3],2))
xgboost1 = xgboost_score_train |> filter(.metric!='mae') |> as.data.frame()
xgboost1 = cbind(xgboost1[,1], xgboost1[,3])
ranfor1 = rf_score_train |> filter(.metric!='mae') |> as.data.frame()
ranfor1 = cbind(ranfor1[,1], ranfor1[,3])

compare1 = c('LR','LR','KNN','KNN','XGB','XGB','RF','RF') |> cbind(rbind(linreg1,knn1,xgboost1,ranfor1))
colnames(compare1) = c('model','estimator','estimate') 
compare1 = as.data.frame(compare1) |> pivot_wider(names_from=estimator, values_from=estimate)
tibble(compare1)

# Get pred. metrics from models based on testing data
# LR
...
As seen on the table, LR has the lowest r-squared estimate and highest RMSE while RF is the opposite. KNN has the second highest RMSE with a moderate r-squared estimate. And XGB has both a high RMSE and r-squared estimate. Random Forest model has the lowest RMSE of 0.62 and highest r-squared estimate. Since we are choosing the best model by RMSE, the Random Forest model is thus the best and final model based on our procedure.
...
are closest and furthest from the observed values? What do you hypothesize are the reasons for the good or bad performance at these locations?

2. What variables might predict where your model performs well or not? For example, are their regions of the country where the model does better or worse? Are there variables
that are not included in this dataset that you think might improve the model performance if they were included in your model?

3. There is interest in developing more cost-effect approaches to monitoring air pollution
on the ground. Two candidates for replacing the use of ground-based monitors are numerical models like CMAQ and satellite-based observations such as AOD. How well do
CMAQ and AOD predict ground-level concentrations of PM2.5? How does the prediction performance of your model change when CMAQ or aod are included (or not included) in
the model?
