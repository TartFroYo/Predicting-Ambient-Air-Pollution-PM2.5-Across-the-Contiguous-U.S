...
# excl. id and value
train_scaled = train[,3:50] |> dplyr::select(where(is.numeric)) |> mutate(across(fips:aod, scale)) |> cbind(state=train$state,county=train$county,city=train$city)
...
# Now from this list, you can select the variables. Mostly, the top 20 variables are what we select. However, this is just an indicative value and may vary based upon the problem.

# take top 20 important variables
import20 = import[1:20,]
```
...
fit1 = model1 |> fit(value ~ ., data=train_scaled)
x=summary(fit1$fit)$coefficients
...
res1 = wf1 |> last_fit(split=dat_split)
res1 |> collect_metrics()


library(MASS)
fit <- lm(value ~ .,data=train_scaled)
