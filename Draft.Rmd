...
=======
# Fit XG Boost model to testing data
>>>>>>> 2310bb92d1fdcf1734f35b771b9f5d5f0dc43279
...
As seen on the table and visual, LR has the highest RMSE and lowest R-squared estimate. KNN has the second highest RMSE with a moderate R-squared estimate. And XGB has the lowest RMSE of 0.69 and the highest R-squared estimate of 0.93. Random Forest model has the both the second lowest RMSE and second highest R-squared estimate. Since we are choosing the best model by the best RMSE, which is the lowest one, the Xgboost model is thus the best and final model based on our procedure. After fitting the Xgboost model to the testing data, we find that its RMSE and R-squared is 1.39 and 0.66, respectively. Since we are trying to predict annual average ambient air pollution of a given monitor, then these statistics mean that the amount of error in our model's predictions is about 1.39 $\mu$g/m$^3$, and that the features we used in our model are able to explain 66% of the variance in the data, which means this model does not fit/predict the data well.
...
Indiana, Ohio, Iowa, and Alabama most frequently compared to the other states. Our model also gives the furthers predictions for California, Colorado, Kentucky, Pennsylvania, and Texas most frequently. We noticed that some of these states where our model's prediction performance was bad were on the larger side. We hypothesize that the reason why our prediction performance is bad could be attributed to the possible increase of variance in ambient air pollution that comes with a larger state area or that the model has poor outlier handling.
