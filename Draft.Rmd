...
train_prediction2 = rf_model_final |> 
  parsnip::fit(formula = value ~ CMAQ+lat+county_area+lon+aod, data=train_processed2) |> 
...
loc = test_prediction |> dplyr::select(.pred, value, CMAQ, lat, lon, county_area, aod, state) |> mutate(diff=round(abs(value-.pred),2)) |> arrange(desc(diff))

# Find number of obs.s per the 3 states
loc |> filter(state=='Oregon' | state=='Iowa' | state=='Virginia') |> group_by(state) |> summarise(mon=n())
```
...
loc |> filter(diff<0.5) |> ggplot(aes(y=state)) + geom_bar()
...
```{r Q3-2}
set.seed(123)
# XGB model w/only cmaq
# Preprocessing recipe
rec6 <- 
  recipes::recipe(value ~ CMAQ, data = training(dat_split)) %>%
  # Convert categorical variables to factors
  recipes::step_string2factor(all_nominal()) %>%
  # Combine low frequency factor levels
  recipes::step_other(all_nominal(), threshold = 0.01) %>%
...
folds6 <- recipes::bake(rec6, new_data = training(dat_split)) %>%  
  rsample::vfold_cv(v = 5)
...
xgboost_wf <- workflows::workflow() %>%
  add_model(xgboost_model) %>% 
  add_formula(value ~ CMAQ)

# Hyperparameter tuning
# Takes a long time to run, 5-8 min
xgboost_tuned <- tune::tune_grid(object = xgboost_wf, resamples = folds6,
  grid = xgboost_grid, metrics = yardstick::metric_set(rmse, rsq),
  control = tune::control_grid(verbose = TRUE))
...
# Eval. model performance on train data
test_processed <- bake(rec6,  new_data = testing(dat_split))
train_processed <- bake(rec6,  new_data = training(dat_split))

test_prediction = xgboost_model_final |> 
  parsnip::fit(formula = value ~ CMAQ, data=train_processed) |> 
  predict(new_data=test_processed) |> 
  bind_cols(testing(dat_split))

res6 = test_prediction |> 
  yardstick::metrics(value, .pred) |> 
  mutate(.estimate=format(round(.estimate, 2), big.mark=","))
res6
```
...
```{r Q3-3}
set.seed(123)
# XGB model w/only aod
# Preprocessing recipe
rec7 <- 
  recipes::recipe(value ~ aod, data = training(dat_split)) %>%
  # Convert categorical variables to factors
  recipes::step_string2factor(all_nominal()) %>%
  # Combine low frequency factor levels
  recipes::step_other(all_nominal(), threshold = 0.01) %>%
...
folds7 <- recipes::bake(rec7, new_data = training(dat_split)) %>%  
  rsample::vfold_cv(v = 5)
...
xgboost_wf <- workflows::workflow() %>%
  add_model(xgboost_model) %>% 
  add_formula(value ~ aod)

# Hyperparameter tuning
# Takes a long time to run, 5-8 min
xgboost_tuned <- tune::tune_grid(object = xgboost_wf, resamples = folds7,
  grid = xgboost_grid, metrics = yardstick::metric_set(rmse, rsq),
  control = tune::control_grid(verbose = TRUE))
...
# Eval. model performance on train data
test_processed <- bake(rec7,  new_data = testing(dat_split))
train_processed <- bake(rec7,  new_data = training(dat_split))

test_prediction = xgboost_model_final |> 
  parsnip::fit(formula = value ~ aod, data=train_processed) |> 
  predict(new_data=test_processed) |> 
  bind_cols(testing(dat_split))

res7 = test_prediction |> 
  yardstick::metrics(value, .pred) |> 
  mutate(.estimate=format(round(.estimate, 2), big.mark=","))
res7
```
