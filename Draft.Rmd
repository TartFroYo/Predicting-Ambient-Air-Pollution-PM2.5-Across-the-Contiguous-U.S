...
As seen on the table, LR has the highest RMSE and lowest r-squared estimate. KNN has the second highest RMSE with a moderate R-squared estimate. And XGB has the lowest RMSE of 0.56 and the highest R-squared estimate of 0.96. Random Forest model has the both the second lowest RMSE and second highest R-squared estimate. Since we are choosing the best model by the best RMSE, which is the lowest one, the Xgboost model is thus the best and final model based on our procedure. After fitting the Xgboost model to the testing data, we find that its RMSE and R-squared is 0.61 and 0.94, respectively. Since we are trying to predict annual average ambient air pollution of a given monitor, then these statistics mean that the amount of error in our model's predictions is about 0.61 $\mu$g/m$^3$, and that the features we used in our model are able to explain 94% of the variance in the data, which means this model fits/predicts the data well.

## Results
```{r Results}
compare |> ggplot(aes(y=rmse)) + geom_bar()
